...
compute_dive_metrics <- function(out_dir = "./processed_datasets/",
  ...
  for(jj in 1:length(dat_rows)){
    ...
    fwrite(dive.met2,
           file = paste0(out_dir,"/dive_metrics-halfway-",jj,".csv"))
  ...
  met.list <- fs::dir_ls(out_dir, glob = "*dive_metrics-halfway*.csv")
  ...
  # input sr.dat: incl columns named dive_dur, surf_dur, id
  sr.dat <- dive.met %>%
  ...
  qq <- quantile(sr.dat$rel.sfc.res,
                 c(0.0005,0.995),
                 na.rm=TRUE)

  sr.dat$log.sfc.res[sr.dat$rel.sfc.res < qq[1]] <- log(qq[1])
  sr.dat$log.sfc.res[sr.dat$rel.sfc.res > qq[2]] <- log(qq[2])
